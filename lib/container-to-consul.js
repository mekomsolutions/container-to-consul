...
class ContainertoConsul {
  ...
  start() {
    return this.docker.start()
      ...
      .then(keys => {
        const serviceIds = Array.isArray(keys) ? keys : [];
        return Promise.all(serviceIds.map(key => {
  ...
  container2Service(container) {
    ...
    if (Object.keys(lbls).length === 0 || lbls['consul.register'] === 'false' || !lbls['consul.register']) {
